...
@Singleton
@ProvidesFor(platform = "multipaper", types = {PlatformSignManagement.class, SignManagement.class})
public class MultiPaperSignManagement extends PlatformSignManagement<Player, Location, String> {
  ...
  @Inject
  protected MultiPaperSignManagement(
      ...
      if (entry != null) {
        ...
        if (conf.validAndEnabled()) {
          ...
          for (var sign : this.platformSigns.values()) {
            if (sign.needsUpdates() && sign.exists() && sign instanceof MultiPaperPlatformSign bukkitSign) {
  ...
  protected @NonNull PlatformSign<Player, String> createPlatformSign(@NonNull Sign base) {
    return new MultiPaperPlatformSign(base, this.plugin.getServer(), this.pluginManager);
  }
}
