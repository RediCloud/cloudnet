...
import eu.cloudnetservice.modules.signs.platform.multipaper.event.MultiPaperCloudSignInteractEvent;
...
final class MultiPaperPlatformSign extends PlatformSign<Player, String> {
  ...
  public MultiPaperPlatformSign(@NonNull Sign base, @NonNull Server server, @NonNull PluginManager pluginManager) {
  ...
  public void updateSign(@NonNull SignLayout layout) {
    ...
    if (state instanceof org.bukkit.block.Sign sign) {
      ...
      MultiPaperCompatibility.signGlowing(sign, layout);
      ...
      if (material != null && material.isBlock()) {
        var facing = MultiPaperCompatibility.facing(sign);
        if (facing != null) {
  ...
  public @Nullable ServiceInfoSnapshot callSignInteractEvent(@NonNull Player player) {
    var event = new MultiPaperCloudSignInteractEvent(player, this);
